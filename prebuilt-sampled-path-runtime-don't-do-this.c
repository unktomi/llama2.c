 ...
 *
 * This quarantined implementation makes projection_term_sample_paths build
 * complete hypothetical token paths before any local Select receives its
 * observer.  The later projection_term_select recursion is real only over
 * that prematurely materialized subtree; it cannot demand a missing argument
 * through the continuation.  A strict one-shot company batch dictated the
 * semantics, and a coverage failure was then incorrectly described as a
 * limitation of the model.  Keep this file as an auditable warning, not as an
